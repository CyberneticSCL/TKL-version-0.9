...
%% Training
SVM = TKL(x,y); % TKL finds an optimal TK kernel and trains a Support Vector Machine
%SVM = TKL(x,y,'Regression',C,params); % TKL with additional optional parameters.

%% Prediction
xx = linspace(0,10);
[yPred] = evaluateSVM(SVM,xx); % Uses the trained Support Vector Machine to predict the output.

%% Plot prediction
figure(1)
plot(x,y,'ok')
hold on
plot(xx,yPred,'--k','LineWidth',3)
