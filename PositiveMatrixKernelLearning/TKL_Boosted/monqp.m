...
% function [xnew, lambda, pos] = monqp(H,c,A,b,C,l,verbose,xinit)
%
% min 1/2  x' H x - c' x                 
%  x 
% contrainte   A' x = b                    
...
if isempty(xinit)
   error('xinit should be not empty');  
end
...
%     end
    ...
    if (sum(xnew< 0) > 0 | sum(xnew > C(ind)) > 0)   &  length(ind) > ncc  %  03/01/2002 AR
        ...
        if isempty(indad)
            ...
            marker_tkl = 0;
            [tI indmin] = min(-x(int32(indad))./d(int32(indad)));
        end
        if isempty(indsup)
            tS = tI+1;
        else
            [tS indS] = min((C(ind(int32(indsup)))-x(int32(indsup)))./d(int32(indsup)));       
        end
        if (marker_tkl == 1), tI = tS + 1; end; 
        ...
        if ((marker_mm & (mm < -sqrt(eps)))  | (marker_mmS & (mmS <  -sqrt(eps)))) & nbiter< nbitermax
