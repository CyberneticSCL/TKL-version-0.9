...
from jax.numpy import cumsum as jaxcumsum
...
#     start = time.time()
    ...
    vector3 = jaxcumsum(vector25, axis = 0)
    vector35 = vector3[::-1]
#     end   = time.time()
...
#     start = time.time()
    ...
    vector6 = jaxcumsum(vector4, axis = 0)
    # end   = time.time()
...
def product_subkernel_otimes_lowdim_par(vector, SlUlT, Ul,  chol_diag):  
    ...
    SlUl     -- product Sl*Ul of Matrix2                 ndarray of shape (n_samples, r)
    indeces -- sorted indeces for the data in 1dK      ndarray of shape (n_samples)
    inverse_indeces -- inverse transformation in 1dK   ndarray of shape (n_samples) 
...
#     print('Start', time.time())
    for dim_index in tqdm(np.arange(D, 0, -1)-1): # loop over d iterations
        d = int(dim_index)
        #         print(d)
        if d == D-1:
